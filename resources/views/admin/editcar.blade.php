        ...
        <div class="main">
            ...
            <div class="details">
                <div class="recentOrders" style="width: 150%">
                    <form action="{{ route('admin.updateCar', $car->id_car) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div>
                                ...
                                <div class=" personal">
                                    ...
                                    <div class="card mb-4 mb-xl-0">
                                        ...
                                        <div class="card-body text-center" style="display: flex; flex-direction: column; align-items: center">
                                            ...
                                            <img style="width: 500px" class="img-account-profile rounded-circle mb-2" src="{{ asset('uploads/' . $car->img_car) }}" alt="">
                                            <!-- Profile picture help block-->
                                            <div class="small font-italic text-muted mb-4">JPG or PNG no larger than 5 MB</div>
                                            <!-- Profile picture upload button-->
                                            {{-- <button class="btn btn-primary" type="button">Upload new image</button> --}}
                                            <input type="file" class="btn btn-primary" name="img_car" id="img_car">
                                    ...
                                    <div class="fields">
                                        <div class="input-field">
                                            ...
                                            <input class="inst_crs_inpt" type="text" name="name_car" id="name" value="{{ $car->name_car }}">
                                        </div>

                                        {{-- <div class="input-field">
                                            <label for="inst_crs_tag"  class="inst_crs_det_lbl">Car Description</label>
                                            <textarea class="inst_crs_inpt" name="desc_car" id="desc_car">{{ $car->desc_car }} </textarea>
                                        </div> --}}

                                        <div class="input-field">
                                            <label>Category</label>
                                            <select class="inst_crs_lang" name="id_category">
                                                <option value="{{$car->id_category}}" selected></option>
                                                @foreach ($catCars as $catCar)
                                                @if ($catCar['id_category'] === $car->id_category)
                                                <option value="{{ $catCar['id_category'] }}" selected>{{ $catCar['name_category'] }}</option>
                                                @foreach ($catCars as $catCar)
                                        ...
                                        <div class="input-field">
                                            ...
                                            <input type="number" class="inst_crs_inpt" name="price_car" id="price_car" value="{{ $car->price_car }}">
                                        </div>

                                        <div class="input-field">
                                            <label for="inst_crs_name" class="inst_crs_det_lbl">Car Color</label>
                                            <input class="inst_crs_inpt" type="text" name="color_car" id="color_car" value="{{ $car->color_car }}">
                                        </div>

                                        <div class="input-field">
                                            <label for="inst_crs_name" class="inst_crs_det_lbl">Car Model</label>
                                            <input class="inst_crs_inpt" type="text" name="model_car" id="model_car" value="{{ $car->model_car }}">
                                        </div>

                                        <div class="input-field">
                                            <label for="inst_crs_name" class="inst_crs_det_lbl">Car Type</label>
                                            <input class="inst_crs_inpt" type="text" name="type_car" id="type_car" value="{{ $car->type_car }}">
                                        </div>

                                        <div class="input-field">
                                            <label for="inst_crs_name" class="inst_crs_det_lbl">Car Insurance</label>
                                            <input class="inst_crs_inpt" type="date" name="car_insurance" id="car_insurance" value="{{ $car->car_insurance->format('Y-m-d') }}">
                                        </div>

                                        <div class="input-field">
                                            <label for="inst_crs_name" class="inst_crs_det_lbl">Exp Car Insurance</label>
                                            <input class="inst_crs_inpt" type="date" name="exp_car_insurance" id="exp_car_insurance" value="{{ $car->exp_car_insurance->format('Y-m-d') }}">
                                        </div>

                                        <div class="input-field">
                                            <label for="inst_crs_name" class="inst_crs_det_lbl">Technical Visit</label>
                                            <input class="inst_crs_inpt" type="date" name="technical_visit" id="technical_visit" value="{{ $car->technical_visit->format('Y-m-d') }}">
                                        </div>

                                        <div class="input-field">
                                            <label for="inst_crs_name" class="inst_crs_det_lbl">Exp Technical Visit</label>
                                            <input class="inst_crs_inpt" type="date" name="exp_technical_visit" id="exp_technical_visit" value="{{ $car->exp_technical_visit->format('Y-m-d') }}">
                                        </div>
                ...
                border-radius: 5px;
                color: white;
                text-decoration: none" type="rest" name="submit"  class="inst_crs_rst_btn inst_crs_btns" id="inst_crs_rst_btn">Update</button>
                                            </button>
                                    </div>
                                </div>
                            </div>
